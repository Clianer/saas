...
<script>
    ...
    /*function deleteMany() {
        var ids = $("input:checkbox:checked").length;
        if (ids == 0) {
            alert("请勾选待处理的记录，且每次只能勾选一个")
            return;
        } else {
            if (confirm("你确认要删除此条记录吗？")) {
                ...
                // 删除后返回结果，如果删除成功：重新加载列表。
                $.ajax({
                    url: "/system/dept/delMany?ids="+ ids,
                    type: "post",
                    dataType: "json",
                    success: function (result) {
                        if(result == "ok"){
                        ...
                        <div class="form-group form-inline">
                            <div class="btn-group">
                                <button type="button" class="btn btn-default" title="新建"
                                        onclick='location.href="/system/dept/toAdd.do"'><i class="fa fa-file-o"></i> 新建
                                </button>
